...
<form name="' . $name . '" id="' . $name . '" method="' . $method . '" action="' . $action . '" >' . $the_form_content_hidden . $the_form_content . '
	...
	*/
	function check_input_type($input_def, $input_domain = '')
	{
		...
		if($input_type == 'text')
		{
			$the_input .= wpklikandpay_form::form_input($input_name, $input_id, $input_value, 'text', $input_option);
		}
		elseif($input_type == 'textarea')
		{
			$the_input .= wpklikandpay_form::form_input_textarea($input_name, $input_id, $input_value, $input_option);
		}
		elseif($input_type == 'hidden')
		{
			$the_input .= wpklikandpay_form::form_input($input_name, $input_id, $input_value, 'hidden', $input_option);
		}
		elseif($input_type == 'select')
		{
			$the_input .= wpklikandpay_form::form_input_select($input_name, $input_id, $input_def['possible_value'], $input_value, $input_option, $valueToPut);
		}
		elseif($input_type == 'radio')
		{
			$the_input .= wpklikandpay_form::form_input_check($input_name, $input_id, $input_def['possible_value'], $input_value, 'radio', $input_option);
		}
		elseif($input_type == 'checkbox')
		{
			$the_input .= wpklikandpay_form::form_input_check($input_name, $input_id, $input_def['possible_value'], $input_value, 'checkbox', $input_option);
		}
		elseif($input_type == 'file')
		{
			$the_input .= wpklikandpay_form::form_input($input_name, $input_id, $input_value, 'file', $input_option);
		}
		elseif($input_type == 'gallery')
		{
			$the_input .= wpklikandpay_form::form_input($input_name, $input_id, $input_value, 'text', 'readonly = "readonly"') . 'Gallery field to check';
		}
	...
	*/
	function form_input($name, $id, $value = '', $type = 'text', $option = '')
	{
		...
		{
			return __('Input type not allowed here in ' . __FILE__ . 'at line ' . __LINE__, 'wpklikandpay');
		}
	}
		...
		if(is_array($content) && (count($content) > 0))
		{
			...
			foreach($content as $index => $datas)
			{
				if(is_object($datas) && (!is_array($comboxOptionToHide) || !in_array($datas->id, $comboxOptionToHide)))
				{
					...
					$dataText = __($datas->name ,'wpklikandpay');
					if(isset($datas->code))
					{
						$dataText = __($datas->code ,'wpklikandpay');
					}
				...
				elseif(!is_array($comboxOptionToHide) || !in_array($datas, $comboxOptionToHide))
				{
					...
					$output .= '<option value="' . $valueToPut . '" ' . $selected . ' >' . __($datas ,'wpklikandpay') . '</option>';
				}
			}
